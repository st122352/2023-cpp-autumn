...
public:
	CGraph();
	CGraph(int vertexes, int edges, int t);
	~CGraph();
	...
	int evolutionTree(int n);
	int findClosestAncestor(int s1, int s2);
...
int main(int argc, char* argv[])
{
	...
	CGraph evolution(0, 0, t);
	int closestAn = evolution.findClosestAncestor(s1, s2);

	std::cout << closestAn << std::endl;
...
CGraph::CGraph(int vertexes, int edges, int t)
	: _vertexes(vertexes), _edges(edges), _matrix(nullptr), _edge(nullptr), t(t)
{
	init();
}
...
int CGraph::evolutionTree(int n)
{
	t = n;
	return t;
}

int CGraph::findClosestAncestor(int s1, int s2)
{
	while (s1 != s2)
	{
		if (s1 < s2)
